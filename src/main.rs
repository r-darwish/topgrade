...
fn run() -> Result<(), Error> {
    ...
    #[cfg(target_os = "linux")]
    {
        if !opt.disable.contains(&Group::System) {
            match &distribution {
                Ok(distribution) => {
    ...
    if !opt.disable.contains(&Group::Emacs) {
        git_repos.insert(base_dirs.home_dir().join(".emacs.d"));
    }

    if !opt.disable.contains(&Group::Vim) {
    ...
    if !opt.disable.contains(&Group::GitRepos) {
    ...
    if !opt.disable.contains(&Group::Gem) {
