...
pub fn run_guix(ctx: &ExecutionContext) -> Result<()>{
    ...
    let output = Command::new(&guix).args(&["pull"]).check_output();
    ...
    if should_upgrade {
        run_type.execute(&guix).args(&["package","-u"]).check_run()
    }else {
        return Err(SkipStep(String::from("Guix Pull Failed, Skipping")).into());
    }
